...
from diffalgos.algos.core import TanhGaussianPolicy, Mlp, soft_update_model1_with_model2, ReplayBuffer,\
    mbpo_target_entropy_dict


class SoftAQAgent(object):
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 hidden_sizes=(256, 256), replay_size=int(1e6), batch_size=256,
                 lr=3e-4, gamma=0.99, polyak=0.995,
                 alpha=0.2, auto_alpha=True, target_entropy='mbpo',
                 start_steps=5000, delay_update_steps='auto',
                 utd_ratio=20, num_Q=2,
    ...
    def get_SA_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        sample_idxs = np.random.choice(self.num_Q, self.num_Q, replace=False)
        with torch.no_grad():
            ...
            next_q_with_log_prob_ave = q_prediction_next_ave - self.alpha * log_prob_a_tilda_next
            y_q_ave = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob_ave
            ...
            next_q_with_log_prob_min = min_q - self.alpha * log_prob_a_tilda_next
            y_q_min = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob_min


            y_q = torch.zeros(size=[self.batch_size,1]).to(self.device)

            for i in range(0, self.batch_size):
                if torch.exp(self.temperature * abs(y_q_min[i])).to(self.device) / (
                        torch.exp(self.temperature * abs(y_q_min[i])).to(self.device) + torch.exp(
                    self.temperature * abs(y_q_ave[i])).to(self.device)) > torch.rand(1).to(self.device):
                    y_q[i] = y_q_ave[i]
                else:
                    y_q[i] = y_q_min[i]
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            ...
            """Q loss"""
            y_q, sample_idxs = self.get_SA_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor)
            q_prediction_list = []
            for q_i in range(self.num_Q):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
                q_prediction_list.append(q_prediction)
