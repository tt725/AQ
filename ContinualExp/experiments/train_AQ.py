...
import time
import sys
from diffalgos.algos.AQ import AQAgent
...
def AQ(env_name, num_Over,
             num_Under, seed=0, epochs='mbpo', steps_per_epoch=1000,
    ...
    agent = AQAgent(env_name, obs_dim, act_dim, act_limit, device,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algorithm_name', type=str, default='AQ')
    parser.add_argument('--data_dir', type=str, default='./data')
    parser.add_argument('--num_Q', type=int, default=2)
    parser.add_argument('--num_Over', type=int, default=2)
    parser.add_argument('--num_Under', type=int, default=2)
    ...
    exp_name_full = '%s' % args.env + '_%s' % args.algorithm_name +\
                    '_N%d' % args.num_Q + '_O%d' % args.num_Over + '_U%d' % args.num_Under

    logger_kwargs = setup_logger_kwargs(exp_name_full, args.seed, args.data_dir)

    AQ(args.env, seed=args.seed, epochs=args.epochs,
             logger_kwargs=logger_kwargs,
         num_Q=args.num_Q,
         num_Over=args.num_Over,
         num_Under=args.num_Under,
