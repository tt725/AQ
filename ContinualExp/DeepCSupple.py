...
def figure(Hopper_m, Hopper_n, Hopper_c, Hopper_a, ant_m, ant_n, ant_c, ant_a, walk_m, walk_n, walk_c, walk_a):
    ...
    for key in ant_m.keys():
        ...
        ax1.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_m[key])
        print(smoothed_mean[-1])
        ax1.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_m[key],
                         alpha=0.5)
    ...
    for key in ant_n.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax2.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_n[key])
        print(smoothed_mean[-1])
        ax2.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_n[key],
                         alpha=0.5)
    ...
    for key in ant_c.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax3.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_c[key])
        print(smoothed_mean[-1])
        ax3.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_c[key],
                         alpha=0.5)
    ...
    for key in ant_a.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax4.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_a[key])
        print(smoothed_mean[-1])
        ax4.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_a[key],
                         alpha=0.5)
    ...
    for key in walk_m.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax5.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_m[key])
        print(smoothed_mean[-1])
        ax5.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_m[key],
                         alpha=0.5)
    ...
    for key in walk_n.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax6.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_n[key])
        print(smoothed_mean[-1])
        ax6.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_n[key],
                         alpha=0.5)
    ...
    for key in walk_c.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax7.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_c[key])
        print(smoothed_mean[-1])
        ax7.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_c[key],
                         alpha=0.5)
    ...
    for key in walk_a.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax8.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_a[key])
        print(smoothed_mean[-1])
        ax8.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_a[key],
                         alpha=0.5)
    ...
    for key in Hopper_m.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax9.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_m[key])
        print(smoothed_mean[-1])
        ax9.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_m[key],
                         alpha=0.5)
    ...
    for key in Hopper_n.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax10.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_n[key])
        print(smoothed_mean[-1])
        ax10.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_n[key],
                         alpha=0.5)
    ...
    for key in Hopper_c.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax12.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_c[key])
        print(smoothed_mean[-1])
        ax12.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_c[key],
                          alpha=0.5)
    ...
    for key in Hopper_a.keys():
        ...
        z = np.ones(len(x_raw))
        smoothed_mean = np.convolve(x_raw, y, 'same') / np.convolve(z, y, 'same')
        ax11.plot(xx, smoothed_mean, linewidth=1.5, label=f"{key}", color=color_a[key])
        print(smoothed_mean[-1])
        ax11.fill_between(xx, smoothed_mean - std_raw.values, smoothed_mean + std_raw.values, color=color_a[key],
                          alpha=0.5)
