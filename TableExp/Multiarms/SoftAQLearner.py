...
class SoftAQLearner:
    ...
    def softmax(self, next_state):
        action_number = Env().action_number(next_state)
        positive = abs(self.Q1[next_state][np.argmax(self.Q1[next_state][:action_number])]) + abs(self.Q2[next_state][np.argmax(self.Q2[next_state][:action_number])])
        negative = abs(self.Q2[next_state][np.argmax(self.Q1[next_state][:action_number])]) + abs(self.Q1[next_state][np.argmax(self.Q2[next_state][:action_number])])
        try:
            ...
            if positive > negative:
                ...
                return True
        return np.random.choice([False, True], p=[i / sum_z_exp for i in z_exp])
    ...
    def learning(self, state, action, reward, next_state, done):
        ...
        if np.random.random() >= 0.5:
            ...
            if not done:
                action_number = Env().action_number(next_state)
                is_positive = self.softmax(next_state)
                if is_positive:
