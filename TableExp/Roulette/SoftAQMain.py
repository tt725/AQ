...
import time
from Env import Env
from SoftAQLearner import SoftAQLearner


def SoftAQLearning(parameter):
    S_leave_P_repeat = np.zeros((1000, 100))
    for repeat in range(1000):
        env = Env()
        agent = SoftAQLearner(parameter=parameter)
        S_leave_P = SoftAQUpdate(env=env, agent=agent)
...
if __name__ == "__main__":
    for parameter in [1,2,5,10,100]:
        log = Logger(
            './Result/log.' + "SoftAQ" + f" ({parameter}) " + (time.strftime("%Y.%m.%d.%H.%M.%S", time.localtime())),
            level='debug')
        SoftAQLearning(parameter=parameter)
