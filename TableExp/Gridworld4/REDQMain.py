...
import time
from Env import Env
from REDQLearner import REDQLearner
...
def REDQLearning():
    ...
    for repeat in range(1000):
        env = Env()
        agent = REDQLearner(number_estimator=NUMBER_ESTIMATOR)
        mean_reward = REDQUpdate(env, agent)
...
if __name__ == "__main__":
    log = Logger(
        './Result/log.' + "REDQ" + " " + (time.strftime("%Y.%m.%d.%H.%M.%S", time.localtime())),
        level='debug')
